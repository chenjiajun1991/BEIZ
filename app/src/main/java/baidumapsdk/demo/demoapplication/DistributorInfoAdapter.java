...
*/
        ...
        if (mType == 0) {
          ...
          List<OverlayDemo.BtySaleInfo>tempList= (List<OverlayDemo.BtySaleInfo>) mItemList;
            name="IMEI：" + tempList.get(position).btyImei;
        }
        else {
            name = "IMEI：" + ((UserView.UserBatInfo)mItemList.get(position)).imei;
        }
        item.setText(name);
