package com.sam.beiz.Calendar.bean;
...
import com.sam.beiz.Calendar.util.DBOpenHelper;
...
public class ScheduleDAO {
	...
	public int save(ScheduleVO scheduleVO){
		...
		values.put("type",scheduleVO.getType());
//		Log.v("savescheduleTypeID",scheduleVO.getScheduleTypeID()+"");
//		Log.v("saveRemindID",scheduleVO.getRemindID()+"");
//		Log.v("savescheduleContent",scheduleVO.getScheduleContent());
//		Log.v("savetype",scheduleVO.getType());
		db.beginTransaction();
		int scheduleID = -1;
		try{
	...
	//	dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		Cursor cursor = db.query("schedule", new String[]{"scheduleID","scheduleTypeID","remindID","scheduleContent","scheduleDate","time","alartime","type"}, "scheduleID=?", new String[]{String.valueOf(scheduleID)}, null, null, null,null);
		if(cursor.moveToFirst()){
			...
			cursor.close();
			db.close();
			return new ScheduleVO(schID,scheduleTypeID,remindID,scheduleContent,scheduleDate,time,alartime,type);
		}
		cursor.close();
		db.close();
		return null;
	...
	public ArrayList<ScheduleVO> getAllSchedule(){
		...
		//dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		Cursor cursor = db.query("schedule", new String[]{"scheduleID","scheduleTypeID","remindID","scheduleContent","scheduleDate","time","alartime","type"}, null, null, null, null, "scheduleID desc",null);
		while(cursor.moveToNext()){
			...
			long alartime=cursor.getLong(cursor.getColumnIndex("alartime"));
			String type=cursor.getString(cursor.getColumnIndex("type"));
			ScheduleVO vo = new ScheduleVO(scheduleID,scheduleTypeID,remindID,scheduleContent,scheduleDate,time,alartime,type);
			list.add(vo);
		}
		cursor.close();
		db.close();
	...
	public void deleteBytextName(String content){
		//dbOpenHelper = new DBOpenHelper(context, "schedules.db");
		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		db.beginTransaction();
		try{
			Log.v("inputcontect",content);
			String table = "schedule";
			String[] columns = new String[] {"scheduleID"};
			String selection = "scheduleContent=?";
			String[] selectionArgs = new String[]{content};
		//	String groupBy = "scheduleID";
			String orderBy = "scheduleID";
			Cursor c = db.query(table, columns, selection, selectionArgs, null, null, orderBy, null);
			int learned=0;
			String id="";
			if(c.moveToFirst()){
			...
			c.close();
			//String id=c.getString(c.getColumnIndex("scheduleID"));
			Log.v("id",id);

			db.delete("schedule", "scheduleID=?", new String[]{id});
			db.delete("scheduletagdate", "scheduleID=?", new String[]{id});
			db.setTransactionSuccessful();
		}finally{
			db.endTransaction();
			db.close();
	...
	public ArrayList<ScheduleVO> getAllSchedulesort(){
		ArrayList<ScheduleVO> list = new ArrayList<ScheduleVO>();

		SQLiteDatabase db = dbOpenHelper.getWritableDatabase();
		Cursor cursor = db.query("schedule", new String[]{"scheduleID","scheduleTypeID","remindID","scheduleContent","scheduleDate","time","alartime","type"},null ,null, null, null, "time desc",null);
		while(cursor.moveToNext()){
			int scheduleID = cursor.getInt(cursor.getColumnIndex("scheduleID"));
			int scheduleTypeID = cursor.getInt(cursor.getColumnIndex("scheduleTypeID"));
			int remindID = cursor.getInt(cursor.getColumnIndex("remindID"));
			...
			ScheduleVO vo = new ScheduleVO(scheduleID,scheduleTypeID,remindID,scheduleContent,scheduleDate,time,alartime,type);
			list.add(vo);
			Log.v("alarttime",String.valueOf(alartime));
			Log.v("timelist",time);
		}
		cursor.close();
		db.close();
